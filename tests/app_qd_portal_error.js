...
(async () => {
  try {
    isPortalSuccess = await openClear(); 
    if (isPortalSuccess) {
		 await performLogin(); 
...
const envValues = {
    ...
    url:"https://daxpcv2-qd.clearhub1.tv1/BC/Product/Modules/SignIn.aspx",
    username:"hmmuser",
 ...
 async function openClear() {
    ...
    try {
		...
		var data = JSON.stringify({"testresult":false,"componentname":"portal","location":"Bengaluru","time":new Date().toISOString(),"responseTime":end - start,"failuremessage":"Portal not reachable"});
        data.time = new Date().toISOString();
        callEmitterService(data);
    }
	return portalResult;
